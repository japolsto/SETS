...
- **LIVE demonstration state:** the panel boots disarmed and will not arm without the dry-run acknowledgement. A stored ARMED flag is not restored on load, and writing state removes that key. STOP latches a local PANIC, keeps the first timestamp, and DISARM cannot clear it. LOCAL DEMO RESET returns to DISARMED and states that there is no claim that orders or exposure are resolved. The panic flag survives a storage round-trip. `?panicWebhook=` cannot set a destination, and a previously stored webhook URL is discarded. The page is labeled DEMO / NOT CONNECTED / UI ONLY, shows UNKNOWN when account data is missing, does not draw a $0 P&L marker, uses the loss-intervention-threshold sentence instead of a max-loss brand, does not claim that anyone executes a liquidation, and the new sources contain no Coinbase credentials. These tests do not establish Coinbase isolation.
